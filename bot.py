...
async def date_dialog(update, context):
    text = update.message.text
    my_message = await send_text(update, context, "Подождите пару секунд...⌛")
...
async def date_button(update, context):
    ...
    await send_text(update, context, query)
...
async def opener_dialog(update, context):
    ...
    if (dialog.count == 1):
        ...
        dialog.user["age"] = text
        await send_text(update, context, "Оцените уровень привелкательности: 1-10 баллов")
    elif (dialog.count == 3):
        dialog.user["handsome"] = text
        await send_text(update, context, "Кем он(а) работает?")
    elif (dialog.count == 4):
        dialog.user["occupation"] = text
        await send_text(update, context, "Цель знакомства?")
    elif (dialog.count == 5):
...
async def hello_button(update, context):
    query = update.callback.query.data
    if query == "start":
...
app = ApplicationBuilder().token("7544994187:AAGZB2TM-xLrhNAbQaTXRm-mM9NUsikY6h8").build()
